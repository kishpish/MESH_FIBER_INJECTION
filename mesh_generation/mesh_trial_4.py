...
def repair_with_ftetwild(vertices: np.ndarray, elements: np.ndarray,
    ...
    if len(result) == 2:
        new_verts, new_elems = result
    elif len(result) == 3:
        new_verts, new_elems, _ = result  # Third value might be face info
    else:
        raise ValueError(f"Unexpected return from get_tet_mesh(): {len(result)} values")
...
def repair_mesh(patient_id: str, mesh_dir: str, output_dir: str,
    ...
    try:
        ...
        print(f"\nDEBUG: tools dict = {tools}")
        print(f"DEBUG: tools.get('ftetwild') = {tools.get('ftetwild')}")
        ...
        print(f"\nDEBUG: Checking fTetWild stage...")
        print(f"  tools.get('ftetwild') = {tools.get('ftetwild')}")
        print(f"  best_summary['max_aspect_ratio'] = {best_summary['max_aspect_ratio']}")
        print(f"  TARGETS['max_aspect_ratio'] = {TARGETS['max_aspect_ratio']}")
        print(f"  Should trigger? {tools.get('ftetwild') and best_summary['max_aspect_ratio'] > TARGETS['max_aspect_ratio']}")

        if tools.get('ftetwild') and best_summary['max_aspect_ratio'] > TARGETS['max_aspect_ratio']:
            print("\n  Stage 2: fTetWild remesh...")
            print(f"    AR={best_summary['max_aspect_ratio']:.1f} > target={TARGETS['max_aspect_ratio']}")
            try:
                print("    Calling repair_with_ftetwild...")
                new_verts, new_elems, new_tags = repair_with_ftetwild(
                    vertices, elements, tags,
                    epsilon=1e-3,
                )
        
                print(f"    fTetWild returned {len(new_elems)} elements")
                quality = compute_quality_metrics(new_verts, new_elems)
                summary = summarize_quality(quality)
                ...
                if summary['max_aspect_ratio'] < best_summary['max_aspect_ratio']:
                    ...
                    print(f"    fTetWild didn't improve (AR {summary['max_aspect_ratio']:.1f} vs {best_summary['max_aspect_ratio']:.1f})")
            
            except Exception as e:
                import traceback
                print(f"    fTetWild FAILED with exception: {e}")
                print("    Full traceback:")
                traceback.print_exc()
        else:
            print(f"\n  Stage 2: fTetWild SKIPPED")
            if not tools.get('ftetwild'):
                print(f"    Reason: ftetwild not available (tools['ftetwild']={tools.get('ftetwild')})")
            else:
                print(f"    Reason: AR {best_summary['max_aspect_ratio']:.1f} <= {TARGETS['max_aspect_ratio']}")
